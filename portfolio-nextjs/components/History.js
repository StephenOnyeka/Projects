  ...
  return (
    <div className="relative w-full overflow-hidden rounded-3xl group">
        ...
        <div className='relative z-10 flex flex-col px-6 md:px-12 py-8'>
            {experience.map((item) => (
                <div key={item.year} className='flex justify-between items-center border-b-[0.2px] border-white/20 py-8 hover:bg-white/5 transition-all duration-300 px-4 -mx-4 rounded-xl'>
                    <div className='flex flex-col'>
                        <p className='text-2xl md:text-4xl font-medium pb-1'>{item.title}</p>
                        <a href={item.link} target='_blank' rel="noopener noreferrer" className='text-lg md:text-xl font-light underline opacity-70 hover:opacity-100 transition-opacity'>{item.company} </a>
                    </div>
